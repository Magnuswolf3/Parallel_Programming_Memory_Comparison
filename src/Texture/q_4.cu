...
const char *sampleName = "Texture Convolution";
...
__global__ void transformKernel(float *outputData, int width, int height)
{
    ...
    {
        sum = 0;
        //Sets the starting row and col indexes based off the size of the mask
        int sRow = row - maskRad;
        int sCol = col - maskRad;

        //Iterates through every pixel of the mask
        for(int  i = 0; i < maskSize; i++)
        {
            for(int j = 0; j < maskSize; j++)
            {
                //Gets the current value of the pixel position that will be multiplied
                int currRow = sRow + i;
                int currCol = sCol + j;

                //Checks the position to make sure it isn't out of the image bounds
                if(currRow >= 0 && currRow < height && currCol >= 0 && currCol < width)
                {   
                    //Multiplies the current mask value by the pixel value stored in the texture memory and adds it to the sum
                    sum += tex2D(tex, currCol, currRow) * dMask[i * maskSize + j];
                }
                else {
                    sum = 0;
                }
        ...
        //Sets the value of the current pixel position of the output to the sum calculated
        outputData[(row * width) + col] = clamp(sum);
    }
}

float textureConvolution(unsigned int width, unsigned int height, float* output, char* imagePath, unsigned int size, float* source)
{
    float time;
    ...
    cudaMemcpyToArray(cuArray, 0, 0, source, size, cudaMemcpyHostToDevice);

    // Set texture parameters
    tex.addressMode[0] = cudaAddressModeWrap;
    tex.addressMode[1] = cudaAddressModeWrap;
    tex.filterMode = cudaFilterModePoint; //Make the filter mode point and not linear
    tex.normalized = false;    // Not normalized
...
void runTest(int argc, char **argv)
{
    ...
    std::cout << "Timing texture implementation...\n" << std::endl;
    ...
    time_1 = textureConvolution(width, height, output_1, imagePath, size, hData);
    cudaMemcpyToSymbol(dMask, eMask, maskSize * maskSize * sizeof(float));
    time_2 = textureConvolution(width, height, output_2, imagePath, size, hData);}
    cudaMemcpyToSymbol(dMask, aMask, maskSize * maskSize * sizeof(float));
    time_3 = textureConvolution(width, height, output_3, imagePath, size, hData);
    ...
    if(maskSize == 3)
    {strcpy(outputFilename, imagePath);
    strcpy(outputFilename + strlen(imagePath) - 4, "_out_Sharpened_4.pgm");
    sdkSavePGM(outputFilename, hOutputData_1, width, height);
    printf("Wrote '%s'\n", outputFilename);

    strcpy(outputFilename, imagePath);
    strcpy(outputFilename + strlen(imagePath) - 4, "_out_EdgeDetected_4.pgm");
    sdkSavePGM(outputFilename, hOutputData_2, width, height);
    printf("Wrote '%s'\n", outputFilename);}

    strcpy(outputFilename, imagePath);
    strcpy(outputFilename + strlen(imagePath) - 4, "_out_Averaged_4.pgm");
